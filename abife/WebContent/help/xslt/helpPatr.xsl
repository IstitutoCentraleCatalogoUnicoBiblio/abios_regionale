...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
		...
		<xsl:for-each select="//result/doc">
			...
			<xsl:choose>
				<xsl:when test="position() = 1">
					<xsl:choose>
						<xsl:when test="string-length(substring-after(substring-before(str[@name = 'descrizione'], '$$$'), ';')) = 0">
							<tr>
								<td align="left" bgcolor="#E8FEE7" colspan="2"><font size="2" color="Black" face="Arial">
									<script>
										createRef("<xsl:value-of select="substring-before(str[@name = 'descrizione'], ';')"/>")
									</script>
								</font>
								</td>
							</tr>
							<tr>
								<td align="left" bgcolor="#E8FEE7" colspan="2"><font size="2" color="Black" face="Arial">
									<xsl:text disable-output-escaping="yes">&amp;nbsp;&amp;nbsp;&amp;nbsp;&amp;nbsp;&amp;nbsp;&amp;nbsp;&amp;nbsp;&amp;nbsp;</xsl:text>
										<script>
											createRef("<xsl:value-of select="substring-after(str[@name = 'descrizione'], '$$$')"/>")
										</script>
									</font>
								</td>
							</tr>
						...
						<xsl:otherwise>
							...
							<tr>
								<td align="left" bgcolor="#E8FEE7" colspan="2"><font size="2" color="Black" face="Arial">
									<xsl:text disable-output-escaping="yes">&amp;nbsp;&amp;nbsp;&amp;nbsp;&amp;nbsp;</xsl:text>
									<script>
										createRef("<xsl:value-of select="substring-after(substring-before(str[@name = 'descrizione'], '$$$'), ';')"/>")
									</script>
								</font>
								</td>
							</tr>
				...
				<xsl:otherwise>
					<xsl:choose>
						<xsl:when test="substring-before(str[@name = 'descrizione'], '$$$') = substring-before(//result/doc[number(position()-1)]/str[@name = 'descrizione'], '$$$')">
							<tr>
								<td align="left" bgcolor="#E8FEE7" colspan="2"><font size="2" color="Black" face="Arial">
									<xsl:text disable-output-escaping="yes">&amp;nbsp;&amp;nbsp;&amp;nbsp;&amp;nbsp;&amp;nbsp;&amp;nbsp;&amp;nbsp;&amp;nbsp;</xsl:text>
										<script>
	...
	<xsl:if test="$tot &gt; 0">
			...
			<xsl:if test="$nentries + $from &lt; $tot"><xsl:value-of select="$nentries + $from" /></xsl:if></b> di <b><xsl:value-of select="$tot"/></b> elemento/i
			</font><br/><br/>
	</xsl:if>
	<xsl:if test="$tot = 0">
			<font size="2" face="Arial">
			<b>0</b> elementi trovati.
			</font><br/><br/>
	</xsl:if>
