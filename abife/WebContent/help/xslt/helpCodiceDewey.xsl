...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
		...
		<xsl:for-each select="//result/doc">
			...
			<TR>
				<TD align="left" bgcolor="#E8FEE7" valign="top"><FONT size="1" color="Black" face="Arial">
					<script>
						createRef("<xsl:value-of select="str[@name = 'id_dewey']"/>", "<xsl:value-of select="str[@name = 'descrizione']"/>");
					</script>
					</FONT>
				</TD>
	...
	<xsl:if test="$tot &gt; 0">
			...
			<xsl:if test="$nentries + $from &lt; $tot"><xsl:value-of select="$nentries + $from" /></xsl:if></b> di <b><xsl:value-of select="$tot"/></b> codici Dewey
			</font><br/><br/>
	</xsl:if>
	<xsl:if test="$tot = 0">
			<font size="2" face="Arial">
			<b>0</b> codici Dewey trovati.
			</font><br/><br/>
	</xsl:if>
