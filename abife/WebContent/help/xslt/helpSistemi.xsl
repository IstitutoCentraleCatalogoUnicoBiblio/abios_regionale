...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:if test="$tot &gt; 0">
			...
			<xsl:if test="$nentries + $from &lt; $tot"><xsl:value-of select="$nentries + $from" /></xsl:if></b> di <b><xsl:value-of select="$tot"/></b> sistemi di Biblioteche
			</font><br/><br/>
	</xsl:if>
	<xsl:if test="$tot = 0">
			<font size="2" face="Arial">
			<b>0</b> sistemi di Biblioteche trovati.
			</font><br/><br/>
	</xsl:if>
