...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template name="schema">
		...
		<xsl:for-each select="//result/doc">
			...
			<TR>
				<td align="left" bgcolor="#E8FEE7"><font size="1" color="Black" face="Arial">
					<script language="JavaScript">
						createRef("<xsl:value-of select="str[@name = 'comune']" />","<xsl:value-of select="str[@name = 'provincia']" />","<xsl:value-of select="str[@name = 'regione']" />");
					</script>
					</font>
				</td>
				<td align="left" bgcolor="#E8FEE7"><font size="1" color="Black" face="Arial">
					<xsl:value-of select="str[@name = 'provincia']" />
					</font>
				</td>
				<td align="left" bgcolor="#E8FEE7"><font size="1" color="Black" face="Arial">
	...
	<xsl:if test="$tot &gt; 0">
			...
			<xsl:if test="$nentries + $from &lt; $tot"><xsl:value-of select="$nentries + $from" /></xsl:if></b> di <b><xsl:value-of select="$tot"/></b> comuni
			</font><br/><br/>
	</xsl:if>
	<xsl:if test="$tot = 0">
			<font size="2" face="Arial">
			<b>0</b> comuni trovati.
			</font><br/><br/>
	</xsl:if>
