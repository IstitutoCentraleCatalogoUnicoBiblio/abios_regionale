...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template name="schema">
		...
		<xsl:for-each select="//result/doc">
			...
			<TR>
				<TD align="left" bgcolor="#E8FEE7" valign="top"><FONT size="1" color="Black" face="Arial">
					<xsl:value-of select="str[@name = 'id_dewey']" />
					<!-- xsl:choose>
						<xsl:when test="string-length(str[@name = 'id_dewey']) > 3">
							<xsl:value-of select="substring(str[@name = 'id_dewey'], 1, 3)"/>.<xsl:value-of select="substring(str[@name = 'id_dewey'], 4, 6)"/>
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="str[@name = 'id_dewey']"/>
						</xsl:otherwise>
				...
				<TD align="left" bgcolor="#E8FEE7" valign="top"><FONT size="2" color="Black" face="Arial">
					<script>
						createRef("<xsl:value-of select="str[@name = 'descrizione']"/>","<xsl:value-of select="str[@name = 'id_dewey']"/>")
					</script>
					</FONT>
				</TD>
			</TR>			
	...
	<xsl:if test="$tot &gt; 0">
			...
			<xsl:if test="$nentries + $from &lt; $tot"><xsl:value-of select="$nentries + $from" /></xsl:if></b> di <b><xsl:value-of select="$tot"/></b> classificazioni Dewey
			</font><br/><br/>
	</xsl:if>
	<xsl:if test="$tot = 0">
			<font size="2" face="Arial">
			<b>0</b> classificazioni Dewey trovate.
			</font><br/><br/>
	</xsl:if>
