	...
	<tr>
		<td colspan="2">
			<div class="geocrumbs">
				...
				<c:url value="/mappe/province.jsp" var="crumbregione">
					<c:param name="id_biblioteca" value="*" />
					<c:param name="regione" value="${param.regione}" />
				</c:url>
				<c:url value="/mappe/comuni.jsp?"  var="crumbprovincia">
					<c:param name="id_biblioteca" value="*" />
					<c:param name="regione" value="${param.regione}" />
					<c:param name="provincia" value="${param.provincia}" />
				</c:url>
				
				<a style="font-size: 10px; " href='<c:url value="/mappe/regioni.jsp?id_biblioteca=*"/>'><b> ITALIA </b></a>
				<a style="font-size: 10px; " href="<c:out value="${crumbregione}"/>"><b> -> ${fn:toUpperCase(param.regione)}</b></a>
				<a style="font-size: 10px; " href="<c:out value="${crumbprovincia}"/>"><b> -> ${fn:toUpperCase(param.provincia)}</b></a>
			</div>			
		</td>
	</tr>
	<tr >
		<td colspan="2" align="left" style="border-top: 1px solid #597291">
			...
			<c:url value="/" var="path"></c:url>
				<tr> 
				    ...
				    <td align="left" valign="middle"> 
				      <div class="labelmenu"><b>COMUNI ${fn:toUpperCase(param.provincia)}</b></div>
					  <div class="geomenu">
							<solr:SolrSearch formFields="provincia" start="0" row="0" facetField="comune_record" facetMinCount="1" facetSort="index" facetLimit="-1" extendedQuery="NOT latitudine:0.0 AND NOT longitudine:0.0"/>
				    		<solr:SolrDisplay xsl="mappa_comuni" parameters="path:${path}"/>
					    </div>
				    </td>
				  </tr>
			</table>
