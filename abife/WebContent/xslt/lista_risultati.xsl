...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template name="schema">
		...
		Biblioteche trovate: <b><xsl:value-of select="$max" /></b><br/>
		</font>
		...
		<xsl:for-each select="//result/doc">
			...
			<TR>		
				<TD valign="top" align="left" class="grigioLeft1"><FONT size="1" color="Black" face="Arial">					
					
					<xsl:variable name="mappe">
						<xsl:choose>
							<xsl:when test="(double[@name = 'latitudine'] != 0.0) and (double[@name = 'longitudine'] != 0.0)">true</xsl:when>
							<xsl:otherwise>false</xsl:otherwise>
						</xsl:choose>
					</xsl:variable>
				
					<a><xsl:attribute name="href"><xsl:value-of select="concat('home.jsp?id_biblioteca=',str[@name = 'id_biblioteca'],'&amp;id_comune=',int[@name = 'id_comune'],'&amp;mappe=',$mappe,'&amp;page=zero','&amp;torna=',$torna)" /></xsl:attribute>						
				...
				<xsl:if test="string-length(str[@name = 'stato_catalogazione']) > 0"><xsl:text disable-output-escaping="yes">&amp;nbsp;*</xsl:text></xsl:if>
				</FONT>					
				</TD>
				<TD valign="top" align="left" class="grigioLeft1"><FONT size="1" color="Black" face="Arial">
					<xsl:value-of select="str[@name = 'denominazione_ufficiale']"/></FONT>
				</TD>
				<TD valign="top" align="left" class="grigioLeft1"><FONT size="1" color="Black" face="Arial">
					<xsl:value-of select="str[@name = 'indirizzo']"/>
				</FONT>
				</TD>
				<TD valign="top" align="left" class="grigioLeft1"><xsl:value-of select="str[@name = 'cap']"/></TD>
				<TD valign="top" align="left" class="grigioLeft1"><xsl:value-of select="str[@name = 'comune']"/></TD>
				<TD valign="top" align="left" class="grigioLeft1"><xsl:value-of select="str[@name = 'provincia_sigla']"/></TD>
			</TR>
			<!-- fine singolo record -->
		</xsl:for-each>
	...
	<xsl:if test="$tot &gt; 0">
			...
			<xsl:if test="$nentries + $from &lt; $tot"><xsl:value-of select="$nentries + $from" /></xsl:if></b> di <b><xsl:value-of select="$tot"/></b> biblioteche
			</font><br/><br/>
	</xsl:if>
	<xsl:if test="$tot = 0">
			<font size="2" face="Arial">
			<b>0</b> biblioteche trovate.
			</font><br/><br/>
	</xsl:if>
