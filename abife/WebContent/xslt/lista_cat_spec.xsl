...
<xsl:variable name="speci">
	<xsl:for-each select="arr[@name = 'cataloghi_speciali_display']/str">
...
<tr><td class="verdeCenter3" colspan="13">CATALOGHI SPECIALI</td></tr>

<xsl:choose>
	<xsl:when test="string-length($speci) = 0">
		<tr><td class="grigioCenter2" colspan="13" width="100%"><strong>Nessun dato definito</strong></td></tr>
	</xsl:when>
	<xsl:otherwise>
	<tr><td><xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text></td></tr>
		<tr>
			<td class="verdeCenter1" colspan="2" align="center" width="35%">DESCRIZIONE CATALOGO</td>
			<td class="verdeCenter1" colspan="9" align="center" width="45%">FORMATI / ACCESSO</td>
			<td class="verdeCenter1" colspan="2" align="center" width="20%">COPERTURA BIBLIOGRAFICA</td>
		
		</tr>
		
		<tr>
			<td class="verdinoCenter1" width="22%">Denominazione</td>
			<td class="verdinoCenter1" width="13%">Tipi materiali</td>
			...
			<td class="verdinoCenter1">%</td>
			<td class="verdinoCenter1" width="10%">Da anno</td>
			<td class="verdinoCenter1" width="10%">A anno</td>
		</tr>
		
		<xsl:for-each select="arr[@name = 'cataloghi_speciali_display']/str">
		<tr>
			<td class="grigioLeft1" valign="top" width="22%"><xsl:value-of select="substring-before(substring-after(., 'denominazione:'), ',')"/></td>
			<td class="grigioLeft1" valign="top" width="13%"><xsl:value-of select="substring-before(substring-after(., 'tipi_materiali:'), ',')"/></td>
			...
			<xsl:if test="substring-before(substring-after(., 'supporto_digitale:'), ',') = 'Online' and string-length(substring-after(substring-before(substring-after(., 'supporto_digitale:'), '$$$'), ',')) > 0"><td class="grigioLeft1" valign="top"><a target="_top"><xsl:attribute name="href"><xsl:value-of select="concat('home.jsp?id_biblioteca=', $id_bib, '&amp;page=curl&amp;type=spec&amp;id_cat_url=',substring-after(substring-before(substring-after(., 'supporto_digitale:'), '$$$'), ','),'&amp;torna=', $torna,'&amp;mappe=', $mappe)"/></xsl:attribute>Online</a></td></xsl:if>
			<xsl:if test="substring-before(substring-after(., 'supporto_digitale:'), ',') = 'Online' and string-length(substring-after(substring-before(substring-after(., 'supporto_digitale:'), '$$$'), ',')) = 0"><td class="grigioLeft1" valign="top"><a target="_top"><xsl:attribute name="href"><xsl:value-of select="concat('home.jsp?id_biblioteca=', $id_bib, '&amp;page=curl&amp;type=spec&amp;id_cat_url=0&amp;torna=', $torna,'&amp;mappe=', $mappe)"/></xsl:attribute>Online</a></td></xsl:if>
			...
			<td class="grigioLeft1" valign="top" width="10%"><xsl:value-of select="substring-before(substring-after(., 'da_anno:'), ',')"/></td>
			<td class="grigioLeft1" valign="top" width="10%"><xsl:value-of select="substring-after(., ',a_anno:')"/></td>
		</tr>
		</xsl:for-each>
		
		<tr><td colspan="13"><xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text></td></tr>
</xsl:otherwise>
</xsl:choose>
</tbody></table>
