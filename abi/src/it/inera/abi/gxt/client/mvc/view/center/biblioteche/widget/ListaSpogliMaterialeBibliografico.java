...
public class ListaSpogliMaterialeBibliografico extends ContentPanel {
	...
	private BaseListLoader<ListLoadResult<VoceUnicaModel>> spogliGrigliaLoader;
	...
	public void setGrid() {
		...
		RpcProxy<List<VoceUnicaModel>> spogliGrigliaProxy = new RpcProxy<List<VoceUnicaModel>>() {

			@Override
			protected void load(Object loadConfig, AsyncCallback<List<VoceUnicaModel>> callback) {
				bibliotecheService.getListaSpogliMaterialeBibliograficoByIdBiblio(id_biblioteca, callback);
			}

		};
		
		ModelReader spogliGrigliaReader = new ModelReader();

		spogliGrigliaLoader = new BaseListLoader<ListLoadResult<VoceUnicaModel>>(spogliGrigliaProxy, spogliGrigliaReader);

		final ListStore<VoceUnicaModel> spogliStore = new ListStore<VoceUnicaModel>(spogliGrigliaLoader);

		grid = new Grid<VoceUnicaModel>(spogliStore, cm);
		...
		add.addSelectionListener(new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
				...
				spogliStore.insert(newDescr, 0);
				re.startEditing(spogliStore.indexOf(newDescr), false);
				...
				final Listener<MessageBoxEvent> l = new Listener<MessageBoxEvent>(){
					...
					public void handleEvent(MessageBoxEvent ce) {
						...
						if (btn.getText().equalsIgnoreCase("Si")) {
							int id_rimuoviSpoglio = grid.getSelectionModel().getSelectedItem().getIdRecord();
				...
				final Listener<MessageBoxEvent> l = new Listener<MessageBoxEvent>() {
					...
					public void handleEvent(MessageBoxEvent ce) {
						...
						if (btn.getText().equalsIgnoreCase("Si")) {
							...
							bibliotecheService.addSpoglioMaterialeBibliografico(modelToSave, id_biblioteca, modifica, new AsyncCallback<Void>() {
							...
							if (modifica == false) spogliStore.remove(0);
	...
	public BaseListLoader<ListLoadResult<VoceUnicaModel>> getLoader() {
		...
		return this.spogliGrigliaLoader;
	}
}
