...
/**
 * Classe per la visualizzazione / modifica della lista 
 * di denominazioni alternative
 *
 */
public class ListaDenominazioniAlternativePanel extends ContentPanel {
	...
	private TextField<String> altraDenominazione;
	private ToolBar toolBar = null;
	private Button remove = null;
	private Button add = null;
	private Grid<VoceUnicaModel> grid = null;
	
	public ListaDenominazioniAlternativePanel() {
		...
		/* FINE ALTRE DENOMINAZIONI "ALTERNATIVE" */
	}
	...
	public void setGrid(){
		...
		column.setId("entry");
		column.setHeader("Denominazione alternativa");
		column.setWidth(500);

		altraDenominazione = new TextField<String>();
		altraDenominazione.setAllowBlank(false);
		altraDenominazione.setEmptyText("Inserisci una denominazione...");
		column.setEditor(new CellEditor(altraDenominazione));
		configs.add(column);
		...
		RpcProxy<List<VoceUnicaModel>> denominazioniProxy = new RpcProxy<List<VoceUnicaModel>>() {
			...
			protected void load(Object loadConfig, AsyncCallback<List<VoceUnicaModel>> callback) {
				bibliotecheServiceAsync.getDenominazioniAlternative(id_biblio, callback);
			}
		};
		...
		toolBar.add(new Text("Denominazioni alternative "));
		...
		add.addSelectionListener(new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
				...
				altraDenominazione.clearInvalid();
			}
				...
				final Listener<MessageBoxEvent> l = new Listener<MessageBoxEvent>() {
					public void handleEvent(final MessageBoxEvent me) {
						...
						if (btn.getText().equalsIgnoreCase("Si")) {
							...
							bibliotecheServiceAsync.removeDenominazioneAlternativa(id_denominazione, new AsyncCallback<Void>() {
